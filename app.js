...
function blob(x){return [x.caseNo,x.fault,x.solution,x.system,x.stage,(x.tags||[]).join(' ')].join(' ')}
...
function renderAnswer(text,p,ranked){
  ...
  const bestHtml = best ? `<div class="best-summary"><h4>最相似历史案例</h4><p><b>故障：</b>${escapeHtml(best.fault||'未填写')}</p><p><b>解决方案：</b>${escapeHtml(best.solution||'原表未填写处理方法。')}</p><p><small>系统：${escapeHtml(best.system||'-')} ｜ 案例：${escapeHtml(best.caseNo||'-')} ｜ 匹配分：${best._score}</small></p><button id="copyAi">复制V4分析结果</button></div>` : `<div class="best-summary"><h4>未找到直接历史案例</h4><p>建议补充：故障码、报警颜色、发生工况、漏点位置、设备编号、小时数、是否保内。</p></div>`;
  $('answerBox').classList.remove('hidden');
  $('answerBox').innerHTML = `<div class="answer-head"><div><h2>V4.2 智能分析结果</h2><p>输入内容：${escapeHtml(text)}</p></div><div class="confidence"><span>综合匹配度</span><b>${p.confidence}%</b></div></div>
...
function renderCard(x){
  ...
  tpl.querySelector('.meta').innerHTML = [`案例：${escapeHtml(x.caseNo||'-')}`,`使用阶段：${escapeHtml(x.stage||'已隐藏')}`,`隐私：已隐藏设备编号、地点、日期、服务状态`].join(' ｜ ');
...
function renderStats(){
  ...
  $('stats').innerHTML = `<div><b>${data.length}</b><span>深度脱敏记录</span></div><div><b>${manual.length}</b><span>手册知识</span></div><div><b>${systems.length?escapeHtml(systems[0][0]):'-'}</b><span>最高频系统</span></div><div><b>${stages.length?escapeHtml(stages[0][0]):'-'}</b><span>最高频使用阶段</span></div>`;
}
function renderSide(){
