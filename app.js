...
function buildPlan(input, ranked, manuals){
  ...
  let firstActions = ['记录故障现象、报警颜色、故障码、发生工况和整机编号','查看仪表/诊断系统历史故障，不要先清码','按推荐系统从安全项开始逐项排查'];
...
function init(){
  ...
  $('clearBtn').onclick=()=>{['q','systemFilter','warrantyFilter','locationFilter','tagFilter'].forEach(id=>$(id).value=''); Object.assign(state,{q:'',system:'',warranty:'',location:'',tag:'',ranked:null}); $('answerBox').classList.add('hidden'); render()};
...
function ask(){
  ...
  $('aiHint').textContent = `V4 已匹配 ${manuals.length} 条手册知识、${ranked.length} 条相似历史案例。`;
...
function renderAnswer(text,p,ranked){
  ...
  const bestHtml = best ? `<div class="best-summary"><h4>最相似历史案例</h4><p><b>故障：</b>${escapeHtml(best.fault||'未填写')}</p><p><b>解决方案：</b>${escapeHtml(best.solution||'原表未填写处理方法。')}</p><p><small>系统：${escapeHtml(best.system||'-')} ｜ 地点：${escapeHtml(best.location||'-')} ｜ 整机编号：${escapeHtml(best.serialNo||'-')} ｜ 匹配分：${best._score}</small></p><button id="copyAi">复制V4分析结果</button></div>` : `<div class="best-summary"><h4>未找到直接历史案例</h4><p>建议补充：故障码、报警颜色、发生工况、漏点位置、整机编号、小时数、是否保内。</p></div>`;
  $('answerBox').classList.remove('hidden');
  $('answerBox').innerHTML = `<div class="answer-head"><div><h2>V4 智能分析结果</h2><p>输入内容：${escapeHtml(text)}</p></div><div class="confidence"><span>综合匹配度</span><b>${p.confidence}%</b></div></div>
...
function render(){
  ...
  if(arr.length>120){ const more=document.createElement('div');more.className='empty';more.textContent=`还有 ${arr.length-120} 条未显示，可继续缩小筛选或导出 CSV。`; $('results').appendChild(more); }
}
function renderCard(x){
  const tpl=$('cardTpl').content.cloneNode(true);
  tpl.querySelector('.system').textContent=x.system||'未分类';
  tpl.querySelector('.warranty').textContent=x.warranty||'未填写';
  tpl.querySelector('.score').textContent=x._score?`匹配 ${x._score}`:'记录';
  tpl.querySelector('.fault').textContent=x.fault||'未填写故障描述';
  tpl.querySelector('.meta').innerHTML = [`服务单：${escapeHtml(x.serviceNo||'-')}`,`整机：${escapeHtml(x.serialNo||'-')}`,`地点：${escapeHtml(x.location||'-')}`,`日期：${escapeHtml(x.faultDate||'-')}`,`小时：${escapeHtml(x.hours||'-')}`].join(' ｜ ');
...
function renderStats(){
  ...
  const locations=countBy(data,x=>x.location).slice(0,4);
  $('stats').innerHTML = `<div><b>${data.length}</b><span>故障记录</span></div><div><b>${manual.length}</b><span>手册知识</span></div><div><b>${systems.length?escapeHtml(systems[0][0]):'-'}</b><span>最高频系统</span></div><div><b>${locations.length?escapeHtml(locations[0][0]):'-'}</b><span>最高频地点</span></div>`;
}
function renderSide(){
...
function exportCsv(arr){
  const headers=['服务单号','工作地点','机型','整机编号','交车日期','故障日期','车辆小时数','故障描述','处理方法','故障系统','保内/保外','标签'];
  const rows=arr.map(x=>[x.serviceNo,x.location,x.model,x.serialNo,x.deliveryDate,x.faultDate,x.hours,x.fault,x.solution,x.system,x.warranty,(x.tags||[]).join('|')]);
  const csv=[headers,...rows].map(r=>r.map(v=>`"${String(v??'').replace(/"/g,'""')}"`).join(',')).join('\n');
  const blob=new Blob(['\ufeff'+csv],{type:'text/csv;charset=utf-8;'}); const a=document.createElement('a'); a.href=URL.createObjectURL(blob); a.download='xde130_fault_results_v4.csv'; a.click(); URL.revokeObjectURL(a.href);
}
